...
legend.onAdd = function() {
    ...
    var grades = [1.0, 2.5, 4.0, 5.5, 8.0, 10.0]; // Add the highest magnitude value
    ...
    // Create a gradient object with the same colors as your chooseColor function
    var gradient = L.linearGradient([ [0, 'yellow'], [1, 'red']], 'lineHeight');

    // Use the gradient object to style the legend 
    div.style.background = gradient;

    // Add labels indicating the range of magnitudes
    for (var i = 0; i < grades.length - 1; i++) {
        labels.push(
            '<div style="height: 8px; background-color:' + gradient((i + 1) / (grades.length - 1)) + '">' + grades[i] + (grades[i + 1] ? '&ndash;' + grades[i + 1] + '' : '+') + '</div>'
        );
    }
